...
let _units: OrgUnit[] = [...defaultOrgUnits];
let _members: OrgMember[] = [...defaultOrgMembers];
...
export async function getOrgMembers(): Promise<OrgMember[]> {
  await delay(150);
  return [..._members].sort((a, b) => a.order - b.order);
}

export async function getOrgUnits(): Promise<OrgUnit[]> {
  await delay(100);
  return [..._units].sort((a, b) => a.order - b.order);
}

export async function getActiveOrgUnits(): Promise<OrgUnit[]> {
  await delay(100);
  return _units.filter((u) => u.active).sort((a, b) => a.order - b.order);
}
...
export function getOrgFilterOptions(members: OrgMember[]) {
  return {
    ...
    units: _units.filter((u) => u.active).sort((a, b) => a.order - b.order),
  };
}
...
export async function adminSaveOrgUnit(unit: OrgUnit): Promise<OrgUnit> {
  const idx = _units.findIndex((u) => u.id === unit.id);
  if (idx >= 0) _units[idx] = unit;
  else _units.push(unit);
  return { ...unit };
}

export async function adminDeleteOrgUnit(id: string): Promise<void> {
  _units = _units.filter((u) => u.id !== id);
  _members = _members.map((m) => (m.unitId === id ? { ...m, unitId: '' } : m));
}
...
export async function adminSaveOrgMember(member: OrgMember): Promise<OrgMember> {
  ...
  else _members.push(member);
  return { ...member };
}

export async function adminDeleteOrgMember(id: string): Promise<void> {
  _members = _members.filter((m) => m.id !== id);
  _members = _members.map((m) => (m.parentId === id ? { ...m, parentId: null } : m));
}

export async function adminGetOrgStats() {
  return {
    totalMembers: _members.length,
    activeMembers: _members.filter((m) => m.active).length,
    totalUnits: _units.length,
    activeUnits: _units.filter((u) => u.active).length,
  };
}
